...
import { AppWindow, ArrowDown, ArrowUp, CalendarDays, Clock3, Copy, ExternalLink, Fingerprint, Globe2, History, Link, LockKeyhole, Pencil, Plus, Star, Trash2, Wrench, X } from '@lucide/vue'
...
import VaultPanel from '../vault/components/VaultPanel.vue'

const props = withDefaults(defineProps<{ initialView?: 'overview' | 'tools' }>(), { initialView: 'overview' })
const emit = defineEmits<{ navigate: [category: PanelCategory] }>()
type QuickView = 'overview' | 'copy' | 'links' | 'tools' | 'vault'

const quick = useQuickActionsStore()
const panel = usePanelStore()
const feedback = useFeedbackStore()
const activeView = ref<QuickView>(props.initialView)
...
<template>
  <section class="quick-panel">
    <nav class="quick-tabs" aria-label="效率工具">
      <button type="button" :class="{ active: activeView === 'overview' }" @click="activeView = 'overview'"><History :size="16" />常用</button>
      ...
      <button type="button" :class="{ active: activeView === 'tools' }" @click="activeView = 'tools'"><Wrench :size="16" />转换工具</button>
      <button type="button" :class="{ active: activeView === 'vault' }" @click="activeView = 'vault'"><LockKeyhole :size="16" />加密凭据夹</button>
    </nav>

    <section v-if="activeView === 'overview'" class="quick-section overview-section">
      <div class="entry-grid" aria-label="效率工具快捷入口">
        <button type="button" @click="activeView = 'copy'"><Copy :size="19" /><span><strong>常用工具</strong><small>日期、时间一键复制</small></span></button>
        <button type="button" @click="activeView = 'tools'"><Wrench :size="19" /><span><strong>转换工具</strong><small>编码、时间与二维码</small></span></button>
        <button type="button" @click="activeView = 'vault'"><LockKeyhole :size="19" /><span><strong>加密凭据</strong><small>本地安全保存重要信息</small></span></button>
      </div>
    ...
    <section v-else-if="activeView === 'links'" class="quick-section links-section">
    ...
    <DeveloperToolsPanel v-else-if="activeView === 'tools'" />
...
button:focus-visible, input:focus-visible { outline: 2px solid var(--color-focus); outline-offset: 2px; }@media (max-width: 620px) { .quick-tabs { grid-template-columns: repeat(2, 1fr); }.entry-grid, .favorite-grid, .copy-grid { grid-template-columns: 1fr; }.url-form, .url-form.editing { grid-template-columns: 1fr auto; }.url-form input:first-child { grid-column: 1 / -1; } }
@media (prefers-reduced-motion: reduce) { .entry-grid button:hover { transform: none; } }
</style>
