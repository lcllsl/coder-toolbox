...
import { AppWindow, ArrowDown, ArrowUp, CalendarDays, Clock3, Copy, ExternalLink, Fingerprint, Globe2, History, Link, LockKeyhole, Pencil, Plus, Star, Trash2, X } from '@lucide/vue'
...
type QuickView = 'overview' | 'copy' | 'links' | 'vault'
...
button:focus-visible, input:focus-visible { outline: 2px solid var(--color-focus); outline-offset: 2px; }@media (max-width: 520px) { .quick-tabs { grid-template-columns: repeat(2, 1fr); }.favorite-grid, .copy-grid { grid-template-columns: 1fr; }.url-form, .url-form.editing { grid-template-columns: 1fr auto; }.url-form input:first-child { grid-column: 1 / -1; } }
</style>
