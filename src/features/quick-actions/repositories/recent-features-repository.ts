...
function normalizeRecent(items: (Omit<RecentFeature, 'category'> & { category: string })[]): RecentFeature[] {
  return items.flatMap((item) => {
    const category = normalizePanelCategory(item.category)
    return category ? [{ ...item, category }] : []
  })
}

export async function listRecentFeatures(): Promise<RecentFeature[]> {
  if (!isTauriRuntime()) return normalizeRecent(browserItems()).sort((a, b) => b.usedAt.localeCompare(a.usedAt)).slice(0, 8)
  ...
  return normalizeRecent(rows.map((row) => ({ ...row, useCount: Number(row.useCount) })))
}
