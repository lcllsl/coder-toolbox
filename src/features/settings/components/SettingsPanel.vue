...
import { Bot, Database, Keyboard, LockKeyhole, MonitorUp, MousePointer2, Palette, PauseCircle, Play, Power, RotateCcw, Trash2, VolumeX } from '@lucide/vue'
...
type SettingsView = 'general' | 'behavior' | 'ai' | 'data'
type ClearAction = 'clear-keep' | 'clear-reset'
type ConfirmAction = ClearAction | 'clear-vault-permanent' | 'exit'
...
const includeVaultInClear = ref(false)
const confirmAction = ref<ConfirmAction>()
const pendingClearAction = ref<ClearAction>()
...
const confirmTitle = computed(() => {
  if (confirmAction.value === 'exit') return '退出冒泡？'
  if (confirmAction.value === 'clear-vault-permanent') return '永久删除加密凭据夹？'
  return '清除全部本地数据？'
})
const confirmMessage = computed(() => {
  if (confirmAction.value === 'exit') return '退出后剪贴板监听和健康提醒都会停止，可再次手动启动应用。'
  if (confirmAction.value === 'clear-vault-permanent') return '此操作将永久删除全部加密凭据和凭据夹初始化信息，主密码也无法恢复这些内容。删除后凭据夹会回到第一次使用状态。'
  const vaultScope = includeVaultInClear.value
    ? '已选择加密凭据夹；继续后还需要再次确认，才会永久删除其中的全部凭据。'
    : '加密凭据夹不会被删除。'
  if (confirmAction.value === 'clear-reset') return `将删除普通业务数据并恢复所有设置。原始文件不会被删除。${vaultScope}`
  return `将删除普通业务数据，当前设置和收藏会保留。原始文件不会被删除。${vaultScope}`
})
const confirmLabel = computed(() => {
  if (confirmAction.value === 'exit') return '退出'
  if (confirmAction.value === 'clear-vault-permanent') return '永久删除'
  return '确认清除'
})
...
async function setVaultAutoLock(rawMinutes: string) {
  saving.value = true
  try {
    await settings.setVaultAutoLockMinutes(Number(rawMinutes))
    feedback.notify('凭据夹自动锁定时间已更新', 'success')
  } catch {
    feedback.notify('自动锁定时间保存失败', 'warning')
  } finally { saving.value = false }
}
...
function openClearConfirmation(action: ClearAction) {
  pendingClearAction.value = undefined
...
async function confirmDangerousAction() {
  ...
  if (action !== 'clear-vault-permanent' && includeVaultInClear.value) {
    pendingClearAction.value = action
    confirmAction.value = undefined
    await nextTick()
    confirmAction.value = 'clear-vault-permanent'
    return
  }
  const clearAction = action === 'clear-vault-permanent' ? pendingClearAction.value : action
  const shouldResetVault = action === 'clear-vault-permanent'
  confirmAction.value = undefined
  pendingClearAction.value = undefined
  if (!clearAction) return
  saving.value = true
  try {
    await settings.clearLocalData(clearAction === 'clear-reset')
    if (shouldResetVault) await vault.resetVault()
    shortcutInput.value = settings.settings.globalShortcut
    if (shouldResetVault) includeVaultInClear.value = false
    const result = clearAction === 'clear-reset' ? '本地数据已清除，设置已重置' : '本地数据已清除，设置已保留'
    feedback.notify(shouldResetVault ? `${result}；加密凭据夹已永久删除` : result, 'success')
...
<template>
  <section class="settings-panel">
    ...
    <section v-else-if="activeView === 'behavior'" class="settings-section">
      ...
      <div class="setting-block"><label for="double-click-action"><strong>双击悬浮球</strong><small>启用后，单击会等待 280ms 以区分双击。</small></label><select id="double-click-action" :value="settings.settings.doubleClickAction" @change="setDoubleClick(($event.target as HTMLSelectElement).value as DoubleClickAction)"><option value="none">不设置双击动作</option><option value="quick-actions">打开效率工具</option><option value="clipboard">打开剪贴板</option><option value="dev-tools">打开效率工具·转换工具</option></select></div>
      <div class="setting-block"><label for="vault-auto-lock"><strong>凭据夹自动锁定</strong><small>无凭据操作达到设定时间后，清除当前解锁状态并重新要求主密码。</small></label><select id="vault-auto-lock" :value="settings.settings.vaultAutoLockMinutes" :disabled="saving" @change="setVaultAutoLock(($event.target as HTMLSelectElement).value)"><option value="1">1 分钟</option><option value="5">5 分钟</option><option value="15">15 分钟</option><option value="30">30 分钟</option></select></div>
    </section>

    <AiServiceSettings v-else-if="activeView === 'ai'" />

    <section v-else class="settings-section data-section">
      <header><h2>本地数据</h2><p>普通业务数据始终属于清理范围；加密凭据夹只有主动选择并额外确认后才会删除。</p></header>
      <div class="data-summary"><Database :size="23" /><div><strong>清理范围</strong><p>清理操作不会删除用户的原始文件。</p></div></div>
      <div class="clear-scope" role="group" aria-label="本地数据清理范围">
        <label class="clear-scope-option fixed"><input type="checkbox" checked disabled /><span><strong>普通业务数据</strong><small>剪贴板历史与图片缓存、健康提醒统计、临时中转副本和最近使用记录</small></span></label>
        <label class="clear-scope-option vault"><input v-model="includeVaultInClear" type="checkbox" /><LockKeyhole :size="17" /><span><strong>加密凭据夹</strong><small>默认不选择；删除全部凭据后无法恢复，并需要额外确认</small></span></label>
      </div>
      <button class="data-action" type="button" :disabled="saving" @click="openClearConfirmation('clear-keep')"><Trash2 :size="17" /><span><strong>清除数据并保留设置</strong><small>保留主题、收藏、常用入口、快捷键和提醒配置</small></span></button>
      <button class="data-action warning" type="button" :disabled="saving" @click="openClearConfirmation('clear-reset')"><RotateCcw :size="17" /><span><strong>清除数据并重置设置</strong><small>恢复所有默认设置，但不会隐式删除加密凭据夹</small></span></button>
