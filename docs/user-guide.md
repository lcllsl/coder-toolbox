...
服务商当前固定为 DeepSeek。API Key 保存到独立的本机应用数据文件，不进入 SQLite、导出报告或日志。首次使用时输入 Key 并保存；输入框为空时不能保存。保存后可以输入新 Key 替换、测试连接或移除现有 Key。测试连接和生成图表时不会请求系统钥匙串授权。可选择 V4 Flash（默认）或 V4 Pro。
