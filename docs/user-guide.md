...
- 表格整理与洞察：把杂乱文字整理为可编辑表格，并在本机解析 Excel/CSV，用本地规则或可选 AI 规划生成可交互图表报告。

核心功能按离线使用设计，不需要注册账号，也不会把剪贴板、凭据或转换内容自动上传到网络。AI 办公功能只在用户配置 DeepSeek 并主动提交时联网：智能表格发送用户提交的脱敏文字，Excel 智能图表发送脱敏后的字段概况和少量样例；Excel 文件本身不会上传。
...
- 左上角返回按钮：结束当前面板并重新展开五个花瓣。
- 星标按钮：收藏当前功能分类。
- 齿轮按钮：进入设置。
- 右上角隐藏按钮：暂时隐藏面板，并保留当前页面和未提交输入。

面板显示时，悬浮球会变为隐藏图标。点击悬浮球、面板右上角隐藏按钮，或点击面板之外的区域，都会暂时隐藏面板；悬浮球随后变为恢复图标，再次点击会恢复刚才的面板，不会重新打开分类花瓣。隐藏或返回面板都不会退出应用，健康提醒和剪贴板监听仍会在后台运行。
...
智能表格用于把名单、聊天记录、工作事项、费用、设备或报名信息等杂乱文字整理为结构化表格。它必须使用已配置的 DeepSeek，未配置时会引导前往设置 → AI 服务。

1. 在输入区键入或粘贴文字，也可以点击按钮读取当前剪贴板；按需补充整理目标，并选择自动识别、人员名单、工作事项、费用记录、设备清单或报名信息模板。超过 5 万字符时会提示分批处理，单次上限为 20 万字符。
2. 点击识别后，AI 先只生成表格标题和字段。检查字段名称、类型、说明和顺序，可新增、删除或调整字段；确认后才按固定结构分批提取记录。模型应把原文没有的信息留空，不确定的值会标为待确认。
3. 隐私保护默认开启。手机号、邮箱、证件号、银行卡、密码和 Token 会先在本机替换为占位符，再把脱敏后的完整文字发送给 DeepSeek；首次提交前会显示单独的隐私说明。关闭保护意味着原文可能直接发送，应只处理允许交给该服务的数据。
4. 处理页持续显示已用时间、当前阶段、批次和已验证记录数。结构或记录校验失败时会自动纠正一次；某一批仍失败时，可重试、跳过该批，或停止并查看已经获得的结果。
5. 结果页支持编辑单元格和列名、增删或移动列、删除行、清理完全重复行和全空行、筛选并确认可疑值，以及查看每行对应的原文片段。列名为空或重复时，复制、导出和生成图表会被阻止。
6. 可复制制表符分隔的 TSV，或导出 CSV、XLSX。导出会防止文本被电子表格软件误当公式执行；手机号、证件号和编号保持文本，数字、金额、百分比和日期按相应类型写入。
7. 点击“生成智能图表”可把当前结果直接送入 Excel 智能图表的字段确认与规划页，不需要先生成中间文件。

![智能表格输入与模板选择](images/user-guide/smart-table.png)

智能表格的原文、来源片段、脱敏映射和整理结果只保留在当前运行内存中，不写入 SQLite、设置或日志。关闭页面、刷新或退出应用后无法恢复，请在离开前复制或导出需要的结果。

### 3.5 Excel 智能图表

进入方式：悬浮球 → AI 办公 → Excel 智能图表。

1. 页面首先显示已保存图表。点击已有项目可直接回到字段调整与规划阶段；点击“新建图表”再选择 `.xlsx`、`.xls`、`.csv`，也可把文件拖入面板。文件上限为 50 MB，超过 20 MB 会提示处理可能较慢。从智能表格进入时，当前内存表格会直接作为新数据集使用。
2. 多工作表文件先选择 Sheet；检查系统推断的表头行、字段名称和字段类型，必要时手动修正。字段名称不能为空或重复，改名会同步到报告标题、统计卡片和图例。
3. 未配置 AI 时点击“使用本地方案生成”；已配置 DeepSeek 时可生成 AI 规划。AI 返回结构首次未通过本地校验时会按具体错误自动纠正重试一次，仍失败时显示原因并回退本地方案。
...
服务商当前固定为 DeepSeek。API Key 保存到独立的本机应用数据文件，不进入 SQLite、导出报告或日志。首次使用时输入 Key 并保存；输入框为空时不能保存。保存后可以输入新 Key 替换、测试连接或移除现有 Key。测试连接、智能表格和生成图表时不会请求系统钥匙串授权。可选择 V4 Flash（默认）或 V4 Pro。

### 10.4 数据与应用

“清除数据并保留设置”删除剪贴板历史与图片缓存、提醒统计、临时中转副本、最近使用和已保存智能图表项目，保留主题、收藏、快捷键、提醒配置及常用入口。原始 Excel、CSV、智能表格导出文件和已导出的 HTML 不会被删除；智能表格工作内容本来就不会持久化。
...
- 应用私有目录：剪贴板图片缓存、中转副本和独立的 DeepSeek API Key 配置文件。
- 仅在内存：智能表格原文、来源片段、脱敏映射和整理结果，退出后不保留。
...
核心功能没有账号、云同步、广告或遥测。用户主动点击常用网址时，地址会交给系统浏览器打开。用户主动使用 Excel 智能图表的 AI 规划时，只有界面明示的脱敏 `DataProfile` 会发送给 DeepSeek，Excel 文件和完整工作表不会上传。用户主动使用智能表格时，会发送输入区中的完整脱敏文字；关闭隐私保护后可能发送原文。普通剪贴板历史和加密凭据不会上传。
...
确认已经在设置 → AI 服务中保存 DeepSeek API Key，并使用“测试连接”检查网络和 Key。输入内容不能为空，且不能超过 20 万字符。

### 智能表格结果为什么标为待确认

这通常表示日期或数值含糊、模型置信度较低，或 AI 返回了应用无法安全恢复的未知脱敏占位符。查看对应原文片段，修正单元格后点击确认即可。

### 智能表格关闭后还能恢复吗

不能。原文、脱敏映射和结果只存在于当前运行内存中。请先复制 TSV，或导出 CSV/XLSX。
...
正式验收目标。多显示器缩放与透明命中、图片剪贴板、全局快捷键、开机启动、外部应用、智能表格的 WebView2/剪贴板/文件导出、Windows 凭据剪贴板保护、安装升级卸载及长时间稳定性仍需实机验证。
